	...
	: GameObject(MeshName, MaterialName)
{
	...
	// Set random rotation
	m_Transform.rot = DirectX::XMQuaternionRotationAxis({ 0,1,0 }, (float)rand() / RAND_MAX * DirectX::XM_2PI);

	// Set random bob height
	bob = (float)rand() / RAND_MAX * DirectX::XM_2PI;

	// Set random position
	m_Transform.pos = { (float)rand() / RAND_MAX * 20 - 10, 0, (float)rand() / RAND_MAX * 20 - 10 };
}
